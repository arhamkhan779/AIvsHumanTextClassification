from AIvsHumanTextClassification.entity.config_entity import TrainingConfig
...
class ModelTrainer:
    ...
    def Train_Model_On_Custom_Dataset(self):
        try:
            ...
            X=self.text_processor.fit_transform(self.Data_Frame['text'])
    ...
    def Save_Plot_Results(self):
        try:
            ...
            xticklabels=['Human Text', 'AI Generated'], 
            yticklabels=['Human Text', 'AI Generated'],
            ...
            output_path_cm = 'artifacts/training/confusion_matrix.png'
            plt.tight_layout() 
            plt.savefig(output_path_cm)
            plt.close()

            fpr, tpr, _ = roc_curve(self.Y_test, prediction)
            roc_auc = auc(fpr, tpr)
            RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=roc_auc).plot()
            plt.savefig('artifacts/training/roc_curve.png')
        except Exception as e:
            logger.info(e)
            raise e
